...
import {
  Users, BookOpen, Copy, Check, Plus, X, Send, Clock, Shield,
  ClipboardList, Pill, Image as ImageIcon, CheckCircle2, UserCheck,
  AlertCircle, Zap, Trash2, UploadCloud, Eye, ChevronDown,
} from 'lucide-react';
...
export const CareCircleContent = ({ data, setData, patientId, userEmail, onRefresh }: CareCircleContentProps) => {
  ...
  // Journal view state: default last 7 days; "all" mode adds search + type filter
  const [showAllJournal, setShowAllJournal]       = useState(false);
  const [journalSearch, setJournalSearch]         = useState('');
  const [journalTypeFilter, setJournalTypeFilter] = useState<keyof typeof JOURNAL_TYPES | 'all'>('all');
  ...
  // Help requests view state: default hides "done"; "all" mode shows filter pills
  ...
  const handleInitialize = async () => {
    ...
    try {
      ...
      if (res.ok) onRefresh();
      else setInitError('Failed to initialize circle.');
    } catch { setInitError('Failed to initialize circle.'); }
  ...
  const resetUpload = () => {
    ...
    setUploadDescription(''); setUploadError(null);
    if (fileInputRef.current) fileInputRef.current.value = '';
  };
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-6 mb-6">
        ...
        {/* ── LEFT: Care Circle + Help Requests ── */}
        <div className="space-y-5">
                ...
                return (
                  <div key={cg.email} className="flex items-center gap-3 px-5 py-3.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-[var(--nura-text)] text-sm truncate">{isYou ? 'You' : cg.full_name}</p>
          ...
          <div className="bg-[var(--nura-card)] rounded-3xl border border-white/5 overflow-hidden">
            <div className="flex items-center gap-3 px-5 pt-5 pb-4 border-b border-white/5">
              ...
              {!!data?.help_requests?.filter((r: any) => r.status === 'open').length && (
                <span className="ml-auto text-[10px] font-black px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-400 border border-amber-500/25">
                  {data!.help_requests.filter((r: any) => r.status === 'open').length} open
                </span>
              )}
            </div>
            ...
            <div className="p-4">
                      ...
                      {(['all', 'open', 'claimed', 'done'] as const).map(f => (
                        ...
                        >
                          ...
                          <span className="opacity-60">
                            ({f === 'all' ? data!.help_requests.length : data!.help_requests.filter((r: any) => r.status === f).length})
                          </span>
                        </button>
                      ))}
                    </div>
              ...
              {!data?.help_requests?.length ? (
                ...
                const visible = showAllTasks
                  ? (taskFilter === 'all' ? data!.help_requests : data!.help_requests.filter((r: any) => r.status === taskFilter))
                  : data!.help_requests.filter((r: any) => r.status !== 'done');
                    ...
                    {visible.map((req: any) => {
                      ...
                      return (
                        <div key={req.request_id}
                          className={`bg-[var(--nura-bg)]/40 rounded-2xl border p-3.5 transition-all ${req.status === 'done' ? 'border-emerald-500/10 opacity-70' : 'border-white/5'}`}>
                          <div className="flex items-start justify-between gap-2 mb-2">
                            <p className={`font-bold text-sm leading-tight ${req.status === 'done' ? 'line-through text-[var(--nura-dim)]' : 'text-[var(--nura-text)]'}`}>
                              {req.title}
                            </p>
                            <span className={`shrink-0 text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full border ${sc.bg} ${sc.border} ${sc.color}`}>
                              {sc.label}
                            </span>
                          </div>
                          {req.description && (
                            <p className="text-[var(--nura-dim)] text-xs mb-2 leading-relaxed">{req.description}</p>
                          )}
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <div className="flex gap-1.5">
                              {req.status === 'open' && !isAuthor && (
                                <button onClick={() => handleTaskAction('claim', req.request_id)}
                              ...
                              {req.status === 'claimed' && (isClaimer || isAuthor) && (
                                <button onClick={() => handleTaskAction('complete', req.request_id)}
                              ...
                              {req.status === 'done' && (
                                <span className="text-[9px] text-emerald-400/70 font-bold">✓ Completed</span>
                              )}
                            </div>
                          </div>
                        </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4">
            {!data?.journal?.length ? (
              ...
              if (!showAllJournal) {
                ...
                if (journalTypeFilter !== 'all') filtered = filtered.filter((p: any) => p.type === journalTypeFilter);
                    ...
                    {showAllJournal && (
                      <>
                        ...
                        <div className="flex gap-1.5 flex-wrap">
                          ...
                          {(Object.entries(JOURNAL_TYPES) as [keyof typeof JOURNAL_TYPES, any][]).map(([key, cfg]) => (
                            <button key={key} onClick={() => setJournalTypeFilter(key)}
                              ...
                              <cfg.icon size={8} />
                              {cfg.label} ({data.journal.filter((p: any) => p.type === key).length})
                            </button>
                          ))}
                        </div>
                      </>
        ...
        <div className="bg-[var(--nura-card)] rounded-3xl border border-white/10 p-6 mb-5">
          ...
          {uploadFile && (
            <div className="mt-4 space-y-3">
              <input
                type="text" value={uploadDescription} onChange={e => setUploadDescription(e.target.value)}
        ...
        <div className="fixed inset-0 z-[400] bg-black/70 backdrop-blur-xl flex items-center justify-center p-6">
          <div className="bg-[var(--nura-card)] border border-white/10 rounded-[2.5rem] p-8 w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                onClick={() => { setShowRequestModal(false); setNewReqTitle(''); setNewReqDesc(''); }}
