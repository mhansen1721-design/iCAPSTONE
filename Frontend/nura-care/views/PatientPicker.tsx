...
export const PatientPicker: React.FC<PatientPickerProps> = ({ patients, onSelect, onBack }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-6 animate-in fade-in zoom-in duration-700 bg-[#0b0a1a]">
      ...
      <h1 className="text-5xl font-black text-white mb-3 text-center tracking-tighter">Hello there!</h1>
      <p className="text-indigo-200/50 mb-10 text-center max-w-xs leading-relaxed">Ready to talk? Please select your profile.</p>

      <div className="glass-panel p-10 rounded-[2.5rem] w-full max-w-md border-white/10 shadow-2xl flex flex-col gap-6 bg-[#171140]/40 backdrop-blur-2xl">
        ...
        <div className="space-y-2">
          <select 
            ...
            className="w-full bg-indigo-500/10 border border-white/10 rounded-2xl p-5 text-white font-bold focus:border-indigo-400 outline-none appearance-none cursor-pointer"
          >
            {patients.length === 0 ? (
              <option value="">No profiles found...</option>
            ) : (
              patients.map(p => (
                <option key={p.patient_id || (p as any).id} value={p.patient_id || (p as any).id} className="bg-[#171140] text-white">
                  {p.name || p.full_name}
                </option>
              ))
            )}
        ...
        <div className="grid grid-cols-3 gap-3">
          {[15, 30].map(mins => (
            <button 
              ...
              className={`py-4 rounded-2xl font-black transition-all border ${
                duration === mins ? 'bg-indigo-600 border-indigo-400 text-white shadow-lg shadow-indigo-500/20' : 'bg-white/5 border-white/5 text-indigo-300/60'
              }`}
            >
              {mins}m
            </button>
          ...
          <button 
            ...
            className={`py-4 rounded-2xl font-black transition-all border ${
              duration === 'custom' ? 'bg-indigo-600 border-indigo-400 text-white' : 'bg-white/5 border-white/5 text-indigo-300/60'
            }`}
          >
            Custom
          </button>
        ...
        {duration === 'custom' && (
          <div className="animate-in slide-in-from-top-2 duration-300">
             <input 
              ...
              className="w-full bg-black/20 border border-indigo-500/50 rounded-2xl p-4 text-white text-center font-bold focus:border-indigo-400 outline-none"
             />
          </div>
        )}
        ...
        <button 
          ...
          className="w-full bg-[#3b82f6] hover:bg-blue-600 text-white py-6 rounded-[1.5rem] font-black text-xl shadow-xl shadow-blue-500/20 transition-all flex items-center justify-center gap-3 active:scale-95 disabled:opacity-20"
        >
