...
interface ConfigFlowProps {
  caregiverEmail: string; // Required to link to backend database
...
export const ConfigFlow: React.FC<ConfigFlowProps> = ({ caregiverEmail, patient, onSave, onBack }) => {
  ...
  // --- 1. BACKEND: RETRIEVE EXISTING PROFILE ON LOAD ---
  useEffect(() => {
    const fetchExistingProfile = async () => {
      // Normalize email for URL safety and backend matching
      ...
      try {
        ...
        // Only pre-fill if a profile actually exists in the DB
        if (data.exists && data.patient) {
          const p = data.patient;
          setFormData({
            id: p.patient_id, // Map backend 'patient_id' back to frontend 'id'
            name: p.full_name,
            age: p.age,
        ...
        console.log("Starting fresh: No existing profile found for this user.");
      }
    };
  ...
  if (step === 0) { // Basics
    ...
    // Explicitly check for undefined or non-positive numbers
    if (formData.age === undefined || formData.age <= 0) {
      return "A valid positive age is required.";
    }
    
    // Stage is preset, so we just ensure it exists
  ...
  const handleFinalSave = async () => {
    ...
    // Format data to match the FastAPI Pydantic models exactly
                      ...
                      <div className="space-y-6">
                          <div>
                              ...
                              <input 
                                  ...
                                  className="w-full bg-black/20 border border-white/10 rounded-xl p-4 text-lg text-white focus:outline-none focus:border-indigo-400 transition-all placeholder:text-white/20"
                  ...
                  <div className="glass-panel p-8 rounded-[2rem] border-l-8 border-l-purple-500/50 border-white/10 shadow-xl">
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {[
                          ...
                          { value: DementiaStage.LATE, title: "Severe", desc: "Verbal communication is difficult; responds best to music/visual cues" }
                        ].map((option) => (
                          <button
                            key={option.value}
              ...
              <div className="space-y-6 animate-in slide-in-from-right-8 duration-500 px-2">
                  <div className="glass-panel p-8 rounded-[2rem] border-white/10 shadow-xl">
                      ...
                      <textarea
                          ...
                          className="w-full bg-black/20 border border-white/10 rounded-xl p-4 text-lg text-white min-h-[120px] focus:border-indigo-400 outline-none transition-all"
                          placeholder="Describe their retired life, hobbies, personality..."
                      />
                  </div>
                  ...
                  <div className="glass-panel p-8 rounded-[2rem] border-white/10 shadow-xl border-t-8 border-t-amber-500/50">
                      ...
                      <div className="flex flex-wrap gap-3">
                          {formData.lifestyles.map((tag, idx) => (
                              <span key={idx} className="bg-amber-500/10 border border-amber-500/20 text-amber-100 px-4 py-2 rounded-xl text-base font-bold flex items-center gap-3">
                                  {tag}<button onClick={() => removeTag('lifestyles', idx)}><X size={18}/></button>
                              </span>
                          ))}
                      </div>
                  </div>
          ...
          {activeSection === 'safety' && (
              <div className="space-y-8 animate-in slide-in-from-right-8 duration-500 px-2">
                  ...
                  <div className="glass-panel p-8 rounded-[2rem] border-t-8 border-t-green-500/50 shadow-xl border-white/10">
                      ...
                      <p className="text-base text-indigo-200/70 mb-6">
                        Add safe topics the AI can use to keep the conversation positive and engaging.
                      </p>
                      <div className="flex gap-3 mb-6">
                          <input 
                            type="text" 
                            ...
                            className="flex-1 bg-black/20 border border-white/10 rounded-xl p-4 text-white focus:outline-none focus:border-green-400/50" 
                            placeholder="Add a safe topic..." 
                          />
                          <button 
                            onClick={() => addTag('safeTopics', newTopic, setNewTopic)} 
                            className="bg-green-500/20 px-6 rounded-xl font-bold text-green-200 shadow-sm transition-all active:scale-90"
                          >
                            +
                          </button>
                      </div>
                      <div className="flex flex-wrap gap-3">
                          {formData.safeTopics.map((tag, idx) => (
                              <span key={idx} className="bg-green-500/10 border border-green-500/20 text-green-100 px-4 py-2 rounded-xl font-bold flex items-center gap-3">
                                  {tag}<button onClick={() => removeTag('safeTopics', idx)}><X size={18}/></button>
                              </span>
                          ))}
                      </div>
                  </div>
            
                  {/* Known Triggers Section - Required for Save */}
                  <div className="glass-panel p-8 rounded-[2rem] border-t-8 border-t-red-500/50 shadow-xl border-white/10">
                      ...
                      <p className="text-base text-indigo-200/70 mb-6">
                        Specify words that cause distress. This triggers Tier 3 safety protocols for wandering or medication.
                      </p>
                      <div className="flex gap-3 mb-6">
                          <input 
                            type="text" 
                            ...
                            className="flex-1 bg-black/20 border border-white/10 rounded-xl p-4 text-white focus:outline-none focus:border-red-400/50" 
                            placeholder="Add a trigger..." 
                          />
                          <button 
                            onClick={() => addTag('triggers', newTrigger, setNewTrigger)} 
                            className="bg-red-500/20 px-6 rounded-xl font-bold text-red-200 shadow-sm transition-all active:scale-90"
                          >
                            +
                          </button>
                      </div>
                      <div className="flex flex-wrap gap-3">
                          {formData.triggers.map((tag, idx) => (
                              <span key={idx} className="bg-red-500/10 border border-red-500/20 text-red-100 px-4 py-2 rounded-xl font-bold flex items-center gap-3">
                                  {tag}<button onClick={() => removeTag('triggers', idx)}><X size={18}/></button>
                              </span>
                          ))}
                      </div>
                  </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-[#171140] via-[#171140]/90 to-transparent z-50">
          <div className="max-w-3xl mx-auto flex flex-col gap-4">
              ...
              <div className="flex gap-4 w-full">
                {currentStep > 0 && (
                  <button onClick={handleBackStep} className="px-8 py-5 rounded-[1.5rem] bg-white/10 text-white font-bold border border-white/10 transition-all active:scale-95"><ArrowLeft size={24} /></button>
                )}
                {currentStep < STEPS.length - 1 ? (
                   <button onClick={handleNext} className="flex-1 bg-white text-[#171140] text-xl font-black py-5 rounded-[1.5rem] shadow-2xl transition-all active:scale-95 flex items-center justify-center gap-3">Next: {STEPS[currentStep + 1].label}<ArrowRight size={24} /></button>
                ) : (
                  <button onClick={handleFinalSave} disabled={isLoading} className="flex-1 bg-[#715ffa] hover:bg-[#8475ff] text-white text-xl font-black py-5 rounded-[1.5rem] shadow-2xl transition-all active:scale-95 flex items-center justify-center gap-3">
                      {isLoading ? "Saving..." : "Save Companion Settings"}
